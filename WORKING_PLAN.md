...
- Companies post jobs, find a freelancer, and fund escrow before work begins.
...
- Rule: moderator reviews AI output and closes within 48 hours.
...
- A validated job can be published before funding.
- Escrow funding is required only after a freelancer is selected and before work begins.
...
- Jobs: create brief, validate brief, publish job, list jobs, assign freelancer.
...
- `draft -> validated`: GLM validation score must meet publication threshold.
- `validated -> published`: job becomes visible after GLM validation passes.
- `published -> funded`: freelancer selection plus payment success required.
- `funded -> in_progress`: work may begin only after funding is confirmed.
...
Execution phase order is defined in `workflow.md` and is the authoritative implementation sequence.

Use this file for product rules, contracts, state constraints, and locked assumptions.
...
2. Read `workflow.md` and follow its phase order.
3. Confirm locked decisions are unchanged.
4. Inspect existing schema/routes/tests before edits.
